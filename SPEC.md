...
beacon writes to a small fixed set of surfaces of an iTerm2 window — three on every status change, plus the window title and the pane background in a mode state (`paused` / `release` / `retro` / `done`, RENDER-05). Every other surface is owned by Claude Code, the user's profile, or other tools, and beacon shall not touch them:

```text
  ● ● ●   project · task                              ← §4.8 window title
┌─[ tab ]─────────────────────────────────────────┐ ← §4.6 tab color
...
| Window title | §4.8 | `TITLE` | OS window-title identity (`project · task`) that survives Claude's `/rename`, for a sea of windows (Mission Control, ⌘\`, Dock) | Session `name` set to the interpolated badge template via Apple Events (`set-name`); the base profile disables OSC title-setting (`Allow Title Setting: false`) so Claude's title OSC can't overwrite it, and surfaces the name via `Title Components: 1` |
...
**CLI-18.** When invoked as `beacon-iterm configure`, the CLI shall audit the app-wide iTerm2 Appearance preferences the tab-color signal is tuned for — tabs on the left (`TabViewType=2`, integer), a larger tab-label font (`UseCustomTabBarFontSize=1`, boolean, + `CustomTabBarFontSize=18`, float), taller tabs (`DefaultTabBarHeight=60`, float), and the status bar at the bottom (`StatusBarPosition=1`, integer) — reporting for each whether the current value matches the recommendation, and exit non-zero when any differs. These are global keys in `com.googlecode.iterm2`, not per-profile keys a dynamic profile can carry, so beacon cannot express them in `beacon-dev`. The bare form is **read-only** — the one CLI action that reads iTerm2 preferences rather than writing a surface — and names each drifted setting for the user. `install` (CMD-08) invokes it as a closing advisory step; its non-zero drift exit does not fail the install. HTMLTabTitles is excluded — it serves tab-title *content*, a non-goal (§8), not signal legibility.
