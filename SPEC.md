...
- an **iTerm2 per-pane render adapter** (§4) — paints a single session's state onto its own pane (a status-bar titlebar, the tab color and two-line label, and — opt-in — the badge) so the user can scan many panes without focusing each.
...
beacon writes to a small, fixed set of an iTerm2 window's surfaces: the **status bar** (a per-pane titlebar) and the **tab color** on every status change, the **session name** (which iTerm2 surfaces as both the two-line tab label and the OS window title), and — in a mode state (`paused` / `release` / `retro` / `done`, RENDER-05) — the **pane background**. The **pane badge** is opt-in and off by default (BADGE-15). Every other surface is owned by Claude Code, the user's profile, or other tools, and beacon shall not touch them:

```text
  ● ● ●   project                          ← §4.8 window title (name, line 1)
┌───────────────────────────────────────┐
│ project · task              branch      │ ← §4.4 status-bar titlebar (per pane)
├──[ project / task ]─────────────────────┤ ← §4.6 tab color + §4.8 two-line name label
│   pane content                          │   (§4.3 badge: opt-in, off by default)
└───────────────────────────────────────┘
```

| Area | Section | Namespace | Purpose | Mechanism |
|:---|:---|:---|:---|:---|
| Status bar | §4.4 | `STATUS-BAR` | Per-pane titlebar: a left-aligned `project · task` identity, the tack deliverables beside it, and the branch | Base profile status-bar layout + `SetUserVar` |
| Tab color | §4.6 | `TAB` | Tab-strip status traffic-light, for tabs-not-panes workflows | OSC `SetColors=tab=` for the status traffic-light color |
| Tab label + window title | §4.8 | `TITLE` | `project` / `task` identity — two-line in the tab, line 1 (`project`) in the OS window title (Mission Control, ⌘\`, Dock); survives Claude's `/rename` | Session `name` set to the interpolated title template (`TITLE_FORMAT`, task on a second line) via Apple Events (`set-name`); the base profile disables OSC title-setting (`Allow Title Setting: false`) and surfaces the name via `Title Components: 1` |
...
| Badge | §4.3 | `BADGE` | Opt-in "where am I" + traffic-light status color — **off by default** (BADGE-15); the tab and titlebar carry the identity | When enabled: OSC `SetBadgeFormat` + `SetUserVar` for text, `SetColors=badge=` for color; the base profile (§6.6) carries badge sizing |

beacon shall **not** write to: terminal foreground color, cursor color/shape. The tab *label* and the window title are the same session `name` beacon owns (§4.8) — beacon does not set the tab title through an ad-hoc OSC-1 content channel (§8). The pane background is a narrow exception: painted **only** in a mode state and **only** by swapping into that mode's profile (RENDER-05), never by an ad-hoc background OSC. Tab color is the signal-coloring surface beacon paints on every status change; the badge carries the same logical traffic-light state when enabled (per-pane, visible inside the pane and in Mission Control).
...
**CLI-18.** When invoked as `beacon-iterm configure`, the CLI shall audit the app-wide iTerm2 Appearance preferences the fleet layout is tuned for — tabs on the left (`TabViewType=2`, integer), a larger tab-label font (`UseCustomTabBarFontSize=1`, boolean, + `CustomTabBarFontSize=18`, float), taller tabs for the two-line label (`DefaultTabBarHeight=90`, float), the status bar at the top (`StatusBarPosition=0`, integer), a separate status bar per pane (`SeparateStatusBarsPerPane=1`, boolean), and a raised status-bar height for the titlebar font (`statusBarHeight=44`, float) — reporting for each whether the current value matches the recommendation, and exit non-zero when any differs. These are global keys in `com.googlecode.iterm2`, not per-profile keys a dynamic profile can carry, so beacon cannot express them in `beacon-dev`. The bare form is **read-only** — the one CLI action that reads iTerm2 preferences rather than writing a surface — and names each drifted setting for the user. `install` (CMD-08) invokes it as a closing advisory step; its non-zero drift exit does not fail the install. The tab theme is not audited: recent iTerm2 renders inactive colored tabs as outlines in the default theme, so the earlier Minimal-theme workaround (and its `minimalDeslectedColoredTabAlpha` knob) is unnecessary.
...
The whole section is gated two ways. First on **opt-in**: the badge is **off by default** (BADGE-15) — the tab (color + two-line identity, §4.6 / §4.8) and the status-bar titlebar (§4.4) now carry what the badge used to, so painting it too is redundant in a tabs workflow. The user enables it in config; the machinery below is retained either way. Second, when enabled, on **engagement** (BADGE-14): a pane that has never been the subject of a beacon-aware action shows no badge, so a freshly-opened terminal looks unmanaged. All requirements below describing badge painting, color, and text apply only when the badge is enabled and the pane has engaged.
...
**BADGE-11.** No mode prefixes the badge text with a glyph; a mode's cue is its dynamic profile — the pane background (a color, and for every mode a faint watermark: `||` bars for paused, a rocket for release, a checklist clipboard for retro, a checkered finish-flag for done) — plus the badge color (BADGE-10 / RENDER-05). The badge text is always the raw `project` (and task, BADGE-03), never decorated. This standardizes on background + color as the single mode cue across every surface — pane, dashboard card (WIP-17), and fleet dot (WIP-12) — so nothing has to add, and later strip, a text marker.
...
**BADGE-15.** The badge shall be **opt-in and off by default**. In a tabs workflow the tab (color + two-line identity, §4.6 / §4.8) and the status-bar titlebar (§4.4) already carry what the badge conveyed, so painting it too is redundant. The plugin and shell shall emit no badge OSC (`SetBadgeFormat` / `SetColors=badge=`) and the base profile shall carry an empty `Badge Text` unless the user enables the badge via `"badge": "on"` in the user config (`~/.config/beacon/config.json`). The capability is retained in full — `BADGE_FORMAT`, the color palette, and the `badge-format` / `badge-color` verbs are unchanged — so enabling the toggle restores the BADGE-09 behavior, and a future repurposing (e.g. a lifecycle-phase marker) can reuse the same gate. All other BADGE requirements apply only when the badge is enabled.

### 4.4 Status bar area (STATUS-BAR)

The status bar is a **per-pane titlebar**: a left-aligned `project · task` identity and the current branch. In a per-pane layout it fills the role the OS title bar plays for a window — with *separate status bars per pane* enabled (recommended layout, CLI-18), each split carries its own identity, which the single per-window OS title can't. It is delivered via a beacon-managed dynamic profile the user opts into.

Layout is fixed (no dynamic show/hide beyond the mutually-exclusive branch slot). Chip text color is per-chip in the profile: the identity in a bright near-white, the branch in its hybrid identity/sync hue (STATUS-BAR-03). Value-based coloring beyond the branch requires a custom Python component and is out of scope; the tab color (TAB-01) carries the session's status signal.
...
flowchart TB
    ...
    PRECMD --> S2[uservar beacon_branch_default / clean / diverged / untracked]
    PLUGIN([plugin render on hook])
    PLUGIN --> S1[uservar beacon_project + beacon_task]
    INSTALL([beacon install])
    INSTALL --> PROFILE[Dynamic profile written with titlebar layout]
    PROFILE --> CHIPS[Fixed sequence: project·task identity, spring, branch]
    S1 --> STORE[iTerm2 user var store]
    S2 --> STORE
    STORE --> CHIPS
    CHIPS --> RENDER2[Status-bar titlebar renders per pane]
```
...
**STATUS-BAR-02.** The dynamic profile shall enable the status bar with the following fixed layout, left to right: a **`project · task` identity** flush left, the **deliverables** chip beside it, a **spring**, then the **branch** flush right.

1. **Identity** — `project · task`, the same value the tab label and window title carry (§4.8), rendered in a bright near-white so it reads as the pane's title. Left-aligned; identification only, not clickable. Empty until the pane engages (BADGE-14).
2. **Deliverables** — the session's tack deliverables (STATUS-BAR-04), rendered in purple right of the identity as "what I'm working on." Empty (and collapsed) when tack tracks none.
3. **Spring** — absorbs the slack, pushing the branch to the right edge.
4. **Branch** — the current branch, colored by the hybrid model (STATUS-BAR-03). One of four mutually-exclusive slots renders when in a git repo, none when outside one.

The status bar carries **no action buttons**: the `↖ web` / `⇄ review` / `↗ code` chips of earlier versions are dropped — the fleet dashboard handles navigation and activation, and a per-pane titlebar reads as identity, not a tool strip. The status-bar font is raised (implementation: `Menlo-Regular 28`) so the strip reads as a titlebar; the app-wide status-bar height is raised to match via the recommended layout (CLI-18).

**STATUS-BAR-03.** The branch chip shall color by a **hybrid of branch identity and sync state (#20)**, so the branch you are working pulls the eye while the default branch recedes:

- **default branch** — the repo's default (e.g. `main` / `master`, resolved via `origin/HEAD` when set, else the conventional names) — de-emphasized (muted blue-gray), whatever its sync state;
- a **feature branch**, by sync state — **synced** (cyan), **diverged** (yellow, with a leading `↑N` / `↓N` / `↑N↓M` indicator so a vertical scan spots divergence), **untracked** (orange, no upstream ref).

All Dracula hues (THEME-01); green is absent, reserved for `release`. The four slots are **mutually exclusive** — exactly one renders in a git repo, none outside one — driven by per-slot user-vars the shell publishes, so the profile needs no conditional expressions. The `state → hex` mapping is tunable in one place. Rationale: a fully-synced branch (usually the default) is at rest; the branch carrying pending work (untracked/diverged) is where attention belongs. Empirical iTerm2 quirks that constrain the implementation (SwiftyString comparison expressions being unreliable, hence the per-slot vars) are captured in §6.10.

**STATUS-BAR-04.** The deliverables chip shall surface the session's forge deliverables (issues / PRs / MRs) that **tack** tracks — tack is the deliverable tracker; beacon reads them, it does not accumulate its own. Because a session maps to a tack *route* (not a tack) and deliverables attach to individual tacks — no session→tack edge — beacon isolates by status and time: the route's **`in_progress`** tacks' deliverables, plus tacks **completed since this session started** (`done_at` after the session pin's `started_at`), falling back to `in_progress`-only when the route is location-correlated rather than session-pinned (no `started_at` to compare against). The route is resolved the same way as the `↖ web` URL (session pin → location, `_tack_url_for`), so the chip and the URL never name different work. Labels come from tack (`<repo><sigil><n>`, sigil `#`/`!`/`@`); the current repo's prefix is stripped so a local deliverable reads bare (`#5`) and a cross-repo one stays qualified (`anchor #27`). Each is emitted as a `[label](url)` **markdown link**, joined by `, ` and capped at the most-recent few, so the titlebar renders them clickable (iTerm2 status-bar markdown-link support; on an iTerm2 without it the link degrades to literal `[label](url)` text). Published as `beacon_deliverables` via `SetUserVar` on the same hooks that resolve `beacon_url` (plugin-side — it needs the `tack list --json` subprocess, off the shell hot path); empty and collapsed when tack is absent or the route has no relevant deliverable.

**STATUS-BAR-05.** When the shell prompt redraws, the shell integration shall publish the values the status bar consumes — full project URL, branch text + sync state (with derived per-slot vars, including the default-branch slot, so the profile needs no conditional expressions), local cwd with `~`-substitution, and the resolved URL. The integration also writes the per-session handoff files (§6.5) — now vestigial, since the `↖ web` / `↗ code` action buttons that read them are dropped (STATUS-BAR-02). During a Claude session the shell prompt cannot redraw, so the plugin covers the gap: SessionStart paints the anchor (HOOK-08) and Stop re-resolves chips from the anchor cwd each turn (HOOK-08b) so a new branch or a narrowed URL becomes visible. Between Claude sessions the shell resumes prompt-driven publishing and follows the user's actual PWD. The exact user-var names and handoff-file paths are an implementation contract between the shell snippet, the plugin, and the dynamic profile (see §6.5).
...
The session name carries the session's identity — `project · task` — so a window keeps its project context when Claude Code's `/rename` (or auto-title) would otherwise replace it. iTerm2 surfaces the name two ways: the **tab label** renders it in full — two lines, `project` over `task` (TITLE-05) — and the **OS window title** shows its first line (`project`), the *sea of windows* scanned by Mission Control, ⌘\`, the Dock, and the window bar. The tab carries the whole identity; the window title carries the project.
...
**TITLE-01.** While a Claude session is engaged on an iTerm2 profile, beacon shall set the session `name` to the resolved identity, so the tab shows `project` / `task` (two lines, TITLE-05) and the OS window title shows `project` (line 1). The name shall persist across Claude Code renders — including a `/rename` or an auto-generated title — for the lifetime of the session. Interactive (non-Claude) panes have no task, so the shell integration sets their name to the project when in one and the abbreviated cwd otherwise (a `beacon_title` var carrying that "project else cwd" value, mirroring the badge's value-level cwd fallback per BADGE-04) — otherwise the `Allow Title Setting: false` profile (TITLE-03) would leave a plain shell showing the profile name, or a blank title outside any project.

**TITLE-02.** The window title shall be sourced from the iTerm2 **session name**, *not* the profile name: the base and mode profiles set `Title Components` to the session-name component alone (no job / working-dir / tty, which would surface the long `node …/mcp-remote …` command line of a Claude session), and the session name holds the interpolated title template `\(user.beacon_project)\(user.beacon_task_nl)` (`TITLE_FORMAT`, TITLE-04/-05) on an engaged Claude pane, or `\(user.beacon_title)` on an interactive pane (TITLE-01). The profile `Name` — the switch key `set-profile` matches (§6.6) — is unchanged; profile identity and the title are distinct values.
...
The status bar's chips (STATUS-BAR-02 / STATUS-BAR-05) consume a fixed user-var name set published by this snippet:
...
| `beacon_branch_default` | `beacon_branch` when on the repo's default branch, else empty (STATUS-BAR-03 hybrid) | not on the default branch |
| `beacon_branch_clean` | `beacon_branch` when a *non-default* branch is synced, else empty | n/a |
| `beacon_branch_diverged` | `beacon_branch` when a *non-default* branch is ahead/behind, else empty | n/a |
| `beacon_branch_untracked` | `beacon_branch` when a *non-default* branch has no upstream, else empty | n/a |
...
| `beacon_task_nl` | plugin-only; the task newline-prefixed and two-space-indented (`"\n  <task>"`) for the two-line title/tab (TITLE-05) | no task resolved |
| `beacon_deliverables` | plugin-only; the session's tack deliverables as `, `-joined `[label](url)` markdown links (STATUS-BAR-04) — needs `tack list --json`, so plugin-side, not the shell hot path | tack tracks none |
...
Per-session handoff files (see §6.10 caveat 6) live at `<DATA_DIR>/cache/url-<pane-guid>.txt` and `<DATA_DIR>/cache/cwd-<pane-guid>.txt` — `<pane-guid>` is the GUID segment of `ITERM_SESSION_ID` (§6.10 caveat 6) and `<DATA_DIR>` is resolved per the convergence rule above. These fed the `↖ web` / `↗ code` action buttons, which the titlebar drops (STATUS-BAR-02); the writes are now **vestigial** and pending removal.
...
3. **Status bar coprocess actions don't interpolate `\(user.*)`.** *(Historical: the titlebar redesign drops the action buttons — STATUS-BAR-02 — so the handoff files below are vestigial. The GUID-keying rationale still governs the live `engaged-<pane-guid>` marker and `set-name` / `focus` targeting, caveat 6.)* The `↖ web` and `↗ code` buttons read per-session handoff files (`url-<pane-guid>.txt`, `cwd-<pane-guid>.txt`) under `<DATA_DIR>/cache/`, where `<pane-guid>` is the segment of `ITERM_SESSION_ID` after the last colon. The files key on the GUID, **not** the full `ITERM_SESSION_ID`: iTerm2 rewrites the `wNtNpN` positional prefix when a pane is moved between windows, tabs, or splits, but the GUID is stable for the pane's life — the same handle focus and set-name target (CLI-15, FOCUS-02). Keying on the full id left a moved pane's buttons reading a file the hook had written under the pane's old position, so the `↖ web` button opened a stale fallback and the `↗ code` button silently did nothing. The button shell derives the key with `${ITERM_SESSION_ID##*:}`; the plugin and shell writers mirror it. The shell snippet writes both files on every prompt; the plugin additionally writes both at SessionStart (HOOK-08) and refreshes them on each Stop (HOOK-08b) so the buttons track narrowings of the session anchor (new branch, pinned tack URL). Both buttons launch via macOS `open` (`open "$url"` and `open -a "Visual Studio Code" "$cwd"`) so the action shell's missing interactive `PATH` is moot; when the handoff value is absent, each button surfaces an OS alert naming beacon rather than navigating to a decoy destination or silently no-op'ing (STATUS-BAR-02 chip 1). The action **injects the command's stdout back into the session as typed input**, so the alert path routes `osascript` stdout to `/dev/null` — otherwise the alert's `button returned:OK` result lands on the command line; `open` prints nothing, so the success path needs no redirect.
...
- **Opening the tab title as an ad-hoc content channel.** beacon owns the session **name** (§4.8, TITLE) so the window title carries `project` and survives `/rename`; iTerm2 surfaces that same name as the two-line tab label, so the tab now carries `project` / `task` **as a consequence of the name beacon already sets** — not through a separate OSC-1 tab-title write. What stays out of scope is treating the tab title as an independent content surface (per-tab HTML, turn-text, glyphs): the tab label is the session identity and nothing more, and state still rides the tab *color* (§4.6).
