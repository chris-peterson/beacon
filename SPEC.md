...
A mode's **note** is not a fifth field. It is an argument of the mode, stored with it and cleared with it (STATE-02): only entering a mode writes either, and leaving one drops both. Recall context for the fleet view is `latest_turn`'s job (WIP-11), which is derived from the transcript and needs no cooperation.
...
Each mode accepts an optional note: `beacon pause "waiting for the VPN"`. The note surfaces in the Claude Code status line (STATUSLINE-01) and the fleet view (§3.8); it is never painted on the pane, where there is no room for prose.
...
**HOOK-03.** When Claude requests user attention (Notification hook with matchers `permission_prompt` and `idle_prompt`, configured as separate matcher entries), the plugin shall set `activity = waiting`. Both prompt kinds produce the same red `blocked` badge and no surface distinguishes them — the sticky marker (HOOK-03b) carries a constant rather than the prompt type. The kind is read in exactly one place, and not to paint anything: HOOK-03d consults it to decide whether an idle observation is worth recording at all.
...
**HOOK-08a.** When SessionStart fires with `source` other than `resume` (i.e. `startup` or `clear`), the plugin shall clear stale per-session signals before publishing the anchor — specifically `override.*`, the declared `mode` (with its note), `activity`, `pending-attention`, `latest_turn`, the harvested Claude Code signals (`cc.*`, PROV-09), and the accumulated `deliverables` plus the `deliverables.dropped` record (STATUSLINE-03 / CMD-24, both scoped to one Claude session). It shall also stamp `session_started_at`, the window STATUSLINE-03 scopes acquisition to — the wipe empties the row and the stamp is what keeps acquisition from refilling it from the bound route's earlier work. Rationale: per-session state files key on the pane (the GUID of `ITERM_SESSION_ID`, §6.2), which outlives any single Claude session, so a fresh `claude` invocation or `/clear` in a pane that previously hosted a session ending mid-permission-prompt would otherwise inherit `activity = waiting` + `pending-attention` and render red. `resume` is excluded because resumed sessions continue prior context by design.
...
**HOOK-03d.** When the Notification matcher is `idle_prompt` and the declared mode is stood down (STATE-15), the plugin shall write neither `activity = waiting` nor the `pending-attention` marker. A `permission_prompt` shall apply HOOK-03 in every mode, stood down or not.

Rationale: a stood-down session sits at an idle prompt *by definition*, so the idle timer is guaranteed to fire on it — red would be the resting state of every parked tab rather than a signal, and it would report the one thing its user already knows, since they are the one who parked or finished it. A permission prompt is the opposite: Claude is blocked on a decision the user has not made, which is exactly what red is for. That asymmetry, not the mode, is what selects the two.
...
Claude Code's Notification payload does not carry which prompt kind fired — the matcher that selected the hook is the only place it survives. So the two matcher entries shall each pass the matcher they matched on to the CLI (`hook Notification --kind <matcher>`), making the wiring in `hooks/hooks.yml` (and its `hooks.json` projection) part of this requirement rather than an implementation detail.
...
**STATE-02.** The plugin shall persist a mode's note as part of the mode (RES-07) and expose it in the cross-session export (WIP-01) so the fleet view and dashboard can surface it. The note shall not write a `task` override; the task slot keeps whatever it had. Rationale: a note is typically a sentence or longer, and reusing it as the task signal overflows the tab. The note surfaces in the Claude Code status line (STATUSLINE-01) and the fleet view; it is never painted on the pane.
...
**STATE-03 — retired.** Setting `status paused` snapshotted the resolved project and task into overrides so the identity held still while the session was parked, and STATE-05 then *preserved* them through auto-resume. One pause therefore pinned a session's identity permanently, above every provider (OVR-02). Live state showed the cost: tasks pinned to branch names the session had long since left, and project labels a version and a half stale, all indistinguishable from labels a user chose. The stability it bought is already provided by the last-rendered snapshot the fleet view reads (WIP-01) — a record that cannot outrank a live provider.
...
**WIP-06.** When the user invokes `prune [--since <when>]` (alias `--keep`), the plugin shall keep sessions active within that window and remove all per-session state for the rest (default 30 days; same duration/ISO grammar as `wip --since`), always keeping the current session. This is garbage collection for accumulated pane state — including project-less sessions that never reached SessionStart; a pruned session repaints on its next hook event. The sweep shall also collect the per-pane handoff files, whose shapes it enumerates explicitly rather than inferring — `cwd-<pane-guid>.txt`, `engaged-<pane-guid>`, `ssh-<pane-guid>.txt` (SSH-01), and the `url-<pane-guid>.txt` that 2.0 retired — by each file's own mtime, since the pane GUID is not recoverable from a session hash. An ssh marker normally lives for one command, but a pane killed mid-session leaves one, and a marker nothing collects would make the buttons decline forever.
...
**WIP-12.** Every fleet consumer shall show **both** axes, because a session blocked on the user is blocked whatever mode it declared — and the merged field could surface only one of the two, always the mode.
...
| Window title + tab label, while ssh'd | §4.10 | `SSH` | The pane's identity is the *host* it is on, not the directory it was launched from — `🔗 <host>` over the remote cwd (SSH-01) | Same session `name`, whose interpolated template carries a slot a **remote** host writes over the ssh pty (SSH-05); the status-bar buttons decline rather than act on the local directory (SSH-09) |
...
**CLI-19.** When invoked as `beacon-iterm getvar <session-id> <name>`, the CLI shall print the value of that iTerm2 session's variable, locating the target by enumerating sessions exactly as `set-name` (CLI-15) does and accepting either the bare GUID or a full `ITERM_SESSION_ID`. This is the read counterpart of `set-name` and the only channel by which a value published from a *remote* host reaches a local process (SSH-10): the value lives in iTerm2, and a status-bar action component cannot interpolate `\(user.*)` into its command (§6.10 caveat 3). An unset variable shall print nothing rather than iTerm2's `missing value`, so a caller need not know AppleScript's vocabulary. When no session matches, the CLI shall exit non-zero.
...
**STATUS-BAR-08.** The `↖ web` action button shall open the session's web view by invoking `beacon open-url <cwd>`, reached through the same absolute-interpreter substitution and cwd handoff file as `↗ code` (STATUS-BAR-07), with the same alert-on-failure path. While the pane is on an ssh session the resolution shall not happen at all: the local directory is whatever preceded the ssh, so its repository is not the one on screen and resolving it would open a plausible wrong answer (SSH-09).
...
**RENDER-04.** When the resolved color state changes, the plugin shall set the tab color (and the badge color when the badge is enabled) to that state's hex via OSC — `tab-color` (CLI-11) and `badge-color` (CLI-10). This is always a pure OSC overlay on the base `beacon-dev` profile or on whichever mode profile the pane sits in; a color change never swaps a profile. Only a **mode** change does that (RENDER-05), which is what keeps a permission prompt mid-mode from flickering the background out and back.
...
**TAB-01.** The tab color shall be the logical color state of BADGE-09 (`ready` / `busy` / `blocked` → palette hex) and nothing else — no mode reaches it. It is delivered by `tab-color` (CLI-11) as an OSC write whenever the state changes, alongside the badge color when the badge is enabled (RENDER-04).
...
**TITLE-01.** While a Claude session is engaged on an iTerm2 profile, beacon shall set the session name so the tab label reads `project` over `task` (two lines, TITLE-05) and the single-line OS window title shows `project`. The title shall persist across Claude Code renders — including a `/rename` or an auto-generated title — for the lifetime of the session. Interactive (non-Claude) panes have no task, so the shell integration sets their title to the project when in one and the abbreviated cwd otherwise (a `beacon_title` var carrying that "project else cwd" value, mirroring the badge's value-level cwd fallback per BADGE-04) — or to `🔗 <host>` while an ssh session holds the prompt (SSH-01), which replaces the value rather than sitting beside it, so the slot keeps one writer and stays non-empty — otherwise the `Allow Title Setting: false` profile (TITLE-03) would leave a plain shell showing the profile name, or a blank title outside any project.
...
### 4.10 Remote sessions (SSH)

A pane that is ssh'd into another server used to report the local directory it was launched from, for as long as the session lasted. The cause is structural rather than a defect: while `ssh` holds the foreground the local zsh is blocked in `waitpid` and prints no prompt, so `_beacon_precmd` never fires and every user var keeps the value the last local prompt published. It is the same prompt-cannot-redraw gap HOOK-08b covers for Claude panes, except that ssh has no second writer at all.

Two halves close it, and they are independent — the first works on every host, including ones the user cannot modify.

**SSH-01.** When an interactive shell begins executing an `ssh` command as the first command of the line, the shell integration shall publish that host as the pane's identity, so the tab label and single-line OS window title read `🔗 <host>`. It shall spawn no process to do so. The identity **replaces** line 1 rather than decorating it: with a remote Claude session out of scope (§8), the local project says nothing about what the pane is doing. Because line 1 is replaced and not appended to, the value is composed into `beacon_title` itself, which keeps that slot's single writer and its never-empty guarantee (TITLE-01) rather than needing a conditionally-empty slot beside it.

`preexec` is the only moment available: it is the last thing the local shell runs before it stops printing prompts. Publishing at that moment costs one OSC write and no Apple Event, because iTerm2 re-evaluates the interpolated session name as the var changes (CLI-15) — the template is set once at source time and never re-set for ssh.

**SSH-02.** When that command returns — by any means, including a signal, a suspend, or a dropped connection — the shell integration shall invalidate **every** sentinel it owns and republish the local identity. `precmd` fires exactly once after a foreground job however it ended, so no signal trap is required.

The invalidation shall be unconditional rather than a diff against the sentinels. The local shell cannot observe what the terminal currently displays, and a remote publisher (SSH-05) may have written any slot since it last looked — so there is nothing here to compare against. Left as a diff, the clearing publishes would match their own stale sentinels, short-circuit, and strand the tab on the host it had left.

**SSH-03.** The display host shall be derived from the `ssh` argv **as typed** — skipping flags that take a value in both their separated and attached forms, splitting clustered booleans, and stripping `user@`, an `ssh://` scheme, and a trailing port — with no subprocess and no consultation of `ssh_config`. A dotted name shall reduce to its first label unless it is an address literal, so `build-01.prod.example.com` reads `build-01` while `10.0.1.5` stays whole. The trade is that two hosts differing only by domain collapse to the same label, which a fixed-height tab strip makes the right way round. The command line shall be read from the alias-expanded form zsh supplies, since `alias ssh='ssh -o …'` is common and the as-typed form misses it.

**SSH-04.** An `ssh` invocation carrying a trailing remote command, or one that backgrounds itself (`-f`), shall paint nothing: the pane is not spending time elsewhere, and painting would flicker the tab for the length of one command. A `-N` tunnel shall paint, since it holds the prompt for as long as the tunnel lives. The `ssh` shall be the first command of the line — a pipeline or a loop containing one is not this pane going somewhere.

**SSH-05.** beacon shall provide a sourceable remote snippet, running under both zsh and bash on any POSIX host and requiring no Python, no beacon checkout, and no macOS, which publishes from the remote prompt the remote working directory as line 2 of the tab label, its absolute form for SSH-08, and the status-bar project and branch chips. It works because iTerm2 user vars are set by an escape sequence and escape sequences are bytes on the tty: written on the far side of an ssh connection they flow up the pty into the local iTerm2 untouched.

The snippet shall return immediately when the shell is not interactive or has no terminal. `scp`, `rsync`, `sftp` and `git`-over-ssh all run a remote shell and read its output as protocol, so escape bytes leaking onto a non-interactive channel corrupt transfers rather than merely looking wrong. It shall write to `/dev/tty` rather than stdout, for the reason §6.6 records for the local snippet.

**SSH-06.** The remote snippet shall publish only the slots the local half leaves to it — `beacon_ssh_path_nl`, `beacon_ssh_dir`, and the mirrored chips — and shall emit no `SetProfile`, `SetColors` or `SetBadgeFormat`. Those carry the activity and mode axes, which the local hooks own: a remote host swapping the profile would stomp a declared mode's background (RENDER-05), and a remote tab color would overwrite a permission prompt's red (TAB-01). Line 1 is likewise not the remote's to write — only the local side knows the name the user typed.

**SSH-07.** The remote snippet's per-prompt cost shall be bounded to one `git for-each-ref` inside a repository and no subprocess outside one, plus an encoder subprocess only for a slot whose value actually moved. The encoder shall be resolved once at source time and shall encode bytes in the C locale, since the branch display carries `↑`/`↓` and a UTF-8 locale makes the encoders character-oriented. POSIX sh cannot base64 without a fork the way zsh's parameter expansion can, so the sentinels are what keep an unchanged prompt free.

**SSH-08.** `beacon ssh-install <host>` shall install that snippet idempotently — a marker-delimited block in the remote login shell's rc that sources a beacon-owned file, so an upgrade rewrites the file and leaves the rc diff at three lines — after printing every path it will touch and confirming on the terminal. `--print` shall emit the snippet and touch nothing; `ssh-uninstall` shall remove both the block and the file. It shall never be reachable from a hook, a render, or `install`: it writes to a machine that is not beacon's to assume consent for. A block whose closing marker is missing shall refuse rather than guess where it ends.

**SSH-09.** While an ssh session is active the status-bar action buttons shall not act on the local working directory, which is whatever the pane held before ssh started. `↖ web` shall refuse with an alert naming the host. `↗ code` shall open `vscode-remote://ssh-remote+<host><absolute-path>` when the remote path is published and the configured editor has a remote-URI scheme, and shall otherwise refuse with an alert naming the host — never opening a local path, and never substituting the remote host's default folder, which is a different action wearing this button's label (the objection STATUS-BAR-07 raises against `open -a`, applied to hosts).

**SSH-10.** The remote path shall reach the button through iTerm2's AppleScript `variable` accessor on a session (CLI-19) — the same Apple Events transport `focus` and `set-name` use — and never by the remote writing a local file. An action component cannot interpolate `\(user.*)` into its command (§6.10 caveat 3), so the value must be *fetched* on the click rather than substituted into it; that is one Apple Event on a click and none on a render. iTerm2 answers an unset variable with `missing value` at exit 0, so a caller shall test the value rather than the exit code.
...
Three parties write into this namespace, and which one owns a slot is the contract: the **plugin** alone writes `beacon_project` / `beacon_task*` / `beacon_title_prefix` (BADGE-02), the **local shell** alone writes `beacon_title` (TITLE-01, including its ssh form per SSH-01), the chips are **mirrored** — local shell, plugin, and the remote snippet all write them with identical semantics (§6.4) — and `beacon_ssh_path_nl` / `beacon_ssh_dir` are written by a **remote** host and retired by the local shell (SSH-02, SSH-05).
...
| `beacon_title` | window-title only (TITLE-01), not a status-bar chip; `🔗 <host>` while ssh holds the prompt (SSH-01), else the project name when in a project, else the abbreviated cwd — it floors on the *path* where the chip floors on the directory name, since a title has room for one | never — the cwd is the floor |
| `beacon_ssh_path_nl` | the remote cwd on an indented second line (`"\n  <path>"`) during an ssh session (SSH-05) — the **only** slot a *remote* host writes, and the local shell retires it on return (SSH-02) | not in an ssh session, or the remote snippet isn't installed |
| `beacon_ssh_dir` | the remote cwd as an *absolute* path, for `↗ code`'s `vscode-remote://` URI (SSH-08) — a `~` means nothing to an editor on this side | as above |
...
3. **Status bar coprocess actions don't interpolate `\(user.*)`.** The `↗ code` button therefore reads a per-session handoff file (`cwd-<pane-guid>.txt`) under `<DATA_DIR>/cache/`, where `<pane-guid>` is the segment of `ITERM_SESSION_ID` after the last colon. The file keys on the GUID, **not** the full `ITERM_SESSION_ID`: iTerm2 rewrites the `wNtNpN` positional prefix when a pane is moved between windows, tabs, or splits, but the GUID is stable for the pane's life — the same handle focus and set-name target (CLI-15, FOCUS-02). Keying on the full id left a moved pane's button reading a file the hook had written under the pane's old position, so the button silently did nothing. The button shell derives the key with `${ITERM_SESSION_ID##*:}`; the plugin and shell writers mirror it. The shell snippet writes the file on every prompt; the plugin additionally writes it at SessionStart (HOOK-08) and refreshes it on each Stop (HOOK-08b) so the button tracks narrowings of the session anchor. When the handoff value is absent, the button surfaces an OS alert naming beacon rather than silently no-op'ing. The action **injects the command's stdout back into the session as typed input**, so the alert path routes `osascript` stdout to `/dev/null` — otherwise the alert's `button returned:OK` result lands on the command line.
   ...
   **An action component cannot interpolate a user var, but Apple Events can read one.** These are different questions, and conflating them is what made a remote pane's directory look unreachable. iTerm2's scripting dictionary exposes a `variable` command on `session`, so a *local* process can read a var a *remote* host published — over the same `osascript` transport `focus` and `set-name` already use (`getvar`, CLI-19). That is the channel `↗ code` uses during an ssh session (SSH-10): the remote path is fetched on the click rather than substituted into the button's command, since the remote host has no way to write a local handoff file and no reverse channel that isn't a command-injection surface. iTerm2 answers an unset variable with `missing value` at **exit 0**, so the value is what a caller must test, not the exit code.

   Alternatives weighed and rejected: iTerm2's `it2getvar` and the Python API (a separate opt-in install, a websocket, and a permission toggle — beacon uses neither, caveat 7); profile **triggers** with capture groups (they match *rendered text*, so the remote would have to print junk into scrollback every prompt, and `\1` from remote output becomes local argv); and the shell-integration `RemoteHost=` / `CurrentDir=` sequences, which would populate iTerm2's native `\(hostname)` and `\(path)` but also make iTerm2 believe the pane's cwd is a remote path — feeding the `AWDS Pane Directory` split-inheritance beacon configures (STATUS-BAR-01) a path that does not exist locally.
...
- Shell adapters other than zsh (bash, fish) for the **local** prompt integration — same architectural posture. The *remote* snippet (SSH-05) is deliberately outside this exclusion and runs under both zsh and bash: the host on the far side of an ssh connection is not beacon's to choose, so refusing anything but zsh there would decline to answer on most servers. It is not the cross-machine session sync excluded below either — nothing crosses to disk, and no state is reconciled; a remote shell publishes into the local terminal's own variable namespace and the local shell retires it.
- **Claude Code running on the remote host.** The split is sharp: every OSC-based surface would cross ssh untouched (tab color, badge, user vars, profile swaps), but everything through Apple Events would not — `set-name`, so the whole of TITLE-01..06, and `focus` (FOCUS-02). Worse, the state files would land on the remote filesystem and `ITERM_SESSION_ID` is not exported through ssh, so the session would seed a different hash bucket and be **absent** from local `wip`, the dashboard, and the status line. A remote Claude pane would therefore get a colored tab and no identity, which is why this waits on *Cross-machine session sync* below rather than being an implementation gap.
