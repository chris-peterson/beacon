...
**HOOK-14.** At SessionStart the plugin shall report, as session context, every installed entry point that no longer reaches the loaded plugin root — the `beacon` wrapper on `$PATH` (CMD-13), the `.zshrc` `source` line (CMD-08), and the status-bar buttons baked into each dynamic profile (STATUS-BAR-07 / STATUS-BAR-08). Each is compared by the `scripts/beacon` **path** it reaches, and reported under that path rather than per surface, since one `install` writes them all from one root and they therefore drift together. A path that no longer exists shall be distinguished from one that does: the first names surfaces that now do nothing, the second surfaces running an older version's code. The report shall name `/beacon:install-beacon` (CMD-26) and say why the shell's own `install` cannot serve — run through the stale wrapper it re-points every surface at the path it already names. A surface that was never installed contributes nothing, since absent is not stale; a path under a git checkout is how beacon is developed and shall be reported only once it is gone. The check shall never disturb the session: an unexpected failure is recorded to the error log (DIAG-04) and nothing is printed.
...
**CLI-18.** When invoked as `beacon-iterm configure`, the CLI shall audit the app-wide iTerm2 preferences the tab signal + two-line label are tuned for, reporting for each whether the current value matches the recommendation, and exit non-zero when any differs:
...
**CLI-19.** When invoked as `beacon-iterm remigrate-profiles [--yes]`, the CLI shall clear iTerm2's `NoSyncMigratedDynamicProfileTagToFlag` preference, so that iTerm2's own dynamic-profile migration runs again on its next launch.
