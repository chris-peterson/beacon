...
1. **Announced URL** — the CR a sibling plugin announced during this session, recorded by the PostToolUse handler from a `codes.bridgeai.anchor/cr.opened` line in a tool's stdout (the suite's interop contract). Stored with the project root it was announced in and returned only when the resolving directory shares that root, because this chain resolves against a directory it is handed rather than against the session — `open-url <dir>` is correct in a pane beacon isn't tracking, and session-scoped state alone would answer for an unrelated checkout. This tier is the only one told the answer instead of working it out; it covers a CR opened while the session was running and nothing else, which is what every tier below it is still for
