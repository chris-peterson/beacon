...
**HOOK-03.** When Claude requests user attention (Notification hook with matchers `permission_prompt` and `idle_prompt`, configured as separate matcher entries), the plugin shall set `activity = waiting`. Both prompt kinds produce the same red `blocked` badge; beacon does not distinguish them on the pane.
