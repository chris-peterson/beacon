...
The pause **reason** is free text that needs an always-visible, non-overlapping per-session surface. The iTerm2 badge cannot be one (it overlays scrolling terminal output; clearing the screen is only transient because the agent keeps printing). Claude Code's `statusLine` command renders a footer row Claude *owns* — above its built-in badges, never overlapping output, in any terminal — which is exactly that surface.

**STATUSLINE-01.** beacon shall provide a `beacon statusline` subcommand suitable for Claude Code's `settings.json` `statusLine.command`. It reads Claude Code's status-line JSON on stdin and, **while the session is paused with a note**, prints the pause reason on one line, led by the `⏸` glyph and colored by the paused state; in every other state it prints nothing. The reason is the only signal it carries for now — `project` / `task` / `status` are already on the tab (RENDER-04, TITLE-05), so they are not repeated here. It shall read only per-session state (no network, no `gh`/`glab`) so it stays cheap enough for Claude Code's frequent status-line invocations. `beacon install` shall surface the `statusLine` settings block for the user to add (opt-in; it does not rewrite `settings.json`). This is the home issue #26 (URL as an OSC-8 footer link) and #18 (accumulated deliverables) can extend; today it carries the reason alone.
