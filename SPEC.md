...
1. **Tack-derived URL** — when `tack` is on `$PATH`, select the session's route by the same correlation order the sessions view uses (WIP-02): the session→route pin (the Claude session id in a route's `sessions[]` block, ties broken by latest `started_at`) is authoritative, then location heuristics (`.tack` pin file, branch name, resolved project name). Matching the branch slug alone would miss a route pinned to the session whose slug differs from the branch, so the status-line URL link (STATUSLINE-02) and the sessions-view reference chip (WIP-09) would name different routes. From the selected route, an inner chain of:
