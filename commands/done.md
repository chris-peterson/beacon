...
The CLI swaps the pane into the done profile (near-black "powered off" background with a faint checkered finish-flag watermark, under a dim-gray badge) as it runs, and drops the task from the badge so it shows the project alone; the user sees the change immediately, and your one-line reply just confirms it. Done persists until the session runs `resume` / `clear` or ends — it does not auto-clear on the next prompt.
