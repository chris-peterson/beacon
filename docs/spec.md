...
**WIP-12.** When a session's logical state is `paused`, the **text-only** fleet consumers shall prefix its reason with a `||` pause glyph — the consumer-side anchor that mirrors the on-pane badge glyph (BADGE-11), so a paused row reads as parked independently of its color dot. This applies to the reason-bearing text consumers of the `wip` payload: the human table (`wip`) and the live view (`watch`). The glyph is rendered even when the session has no description, so a parked session always carries the anchor. It is presentation-only: the stored `description` (WIP-01 payload) keeps its raw text, and consumers add the glyph based on the record's `state`. The glyph is the literal two-bar `||`. `done` carries no fleet-view glyph (consistent with its badge, BADGE-11) — it reads by its purple dot and `done` status alone. The reference dashboard (WIP-10) does **not** add the text glyph: it conveys a mode state through the mode-card treatment (WIP-17) — the pane-analog watermark and tint — so the `||` there would merely duplicate the watermark.
...
**WIP-17.** In the reference dashboard (WIP-10) a session in a mode state (`paused` / `wrapping` / `done`) shall carry a mode-card treatment that echoes its iTerm2 pane background (RENDER-05): a muted tint in the mode's hue and, for a mode whose pane paints a watermark image, a large faint centered watermark of the same glyph (`||` for paused, the power-off ring for done; wrapping is tint-only, matching its imageless pane). This is the dashboard's mode cue and stands in for the text `||` glyph the text-only consumers use (WIP-12), so the dashboard does not also render that glyph. It is presentation-only — derived from the record's `state`, needing no new payload field. Because a mode state is a deliberate set-aside, the dashboard shall not hoist such a session into the attention band on a lingering `pending-attention` marker: the mode outranks the attention signal, mirroring the pane's logical-state precedence (BADGE-09a), so a parked session reads as parked rather than as needing the user.
