...
**TITLE-04.** The session name shall be the interpolated badge template (`BADGE_FORMAT`), single-sourced with the badge (BADGE-01) so the two never drift; iTerm2 re-evaluates it as the `beacon_project` / `beacon_task` user vars change, so beacon re-sets it on the first render, whenever a profile swap resets the name (RENDER-05), and once on the first turn boundary (`UserPromptSubmit` / `Stop`) after each (re)engagement — never on the high-frequency tool hooks (`PreToolUse` / `PostToolUse`), for the NFR-performance reason (§5.1). The turn-boundary re-assertion exists because the session name is a single shared surface with no disjoint-slot separation (§6.5): the interactive shell also sets it once on source (TITLE-01), backgrounded, so that write can land *after* the plugin's SessionStart `set-name` and revert an engaged pane to the shell's `\(user.beacon_project_full)` template. By the first turn boundary that launch-time write has settled, and nothing clobbers the name afterward (the shell never re-sources while Claude holds the pane), so the re-assertion is a **one-shot**: a persisted flag fires it once and suppresses it on every later turn, and a fresh-start wipe (HOOK-08a) or disengagement (HOOK-09) clears the flag so the next engagement re-arms it. This reclaims plugin ownership of the Claude-pane title without spending an Apple Event per turn for a launch-only race.
