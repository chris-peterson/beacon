...
**WIP-13.** Each `wip` record shall carry an `agent_color` field — the color the user set with Claude Code's `/color` (PROV-09) — or null when none is set. It is fleet-view metadata for consumer dashboards, not a painted surface: the pane's badge/tab color stays the status traffic-light (BADGE-09), so the user's aesthetic color surfaces only in the fleet view. The reference dashboard (WIP-10) renders it as a small swatch distinct from the status dot, using the raw color name as a CSS color (an unrecognized value simply renders no fill).
