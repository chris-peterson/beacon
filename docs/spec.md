...
**PROV-02a.** When a Claude session's live subprocess cwd has wandered into a different project root than its SessionStart anchor (HOOK-08), and no user task override is set, the plugin shall surface the wandered location — the abbreviated live cwd, `$HOME` substituted with `~` (PROV-06 form) — in the `task` slot, replacing the PR/branch task. The project slot stays pinned to the anchor (BADGE-02); only the task slot reflects the drift, so the badge reads e.g. `beacon: ~/src/ai-sdlc`. Gating is on the resolved project *root*, not the raw cwd: navigation within the anchored project (into a subdirectory) does not displace the branch task. Rationale: a session that cd's out of its home project is doing cross-project work; surfacing where it went is recall context, but the session's identity (the project it belongs to) is still the anchor.
