...
**TITLE-04.** The session name shall be the interpolated badge template (`BADGE_FORMAT`), single-sourced with the badge (BADGE-01) so the two never drift; iTerm2 re-evaluates it as the `beacon_project` / `beacon_task` user vars change, so beacon re-sets it only when a profile swap resets the name (RENDER-05) and on the first render, not on every hook (NFR performance, §5.1).
