...
Both `paused` and `wrapping` are **mode states** (RENDER-05): each owns a dedicated dynamic profile so the whole pane signals the mode. `paused` is a user halt; `wrapping` is a deliberate closing-out phase a session or skill declares. They differ in lifecycle — `paused` auto-resumes on the next prompt (STATE-04), `wrapping` persists until explicitly cleared.
...
Pause is no longer a separate concept; it is one possible status value (`paused`) the user can set, alongside `idle`, `working`, `waiting`, and `wrapping`. `paused` and `wrapping` are mode states that own a dedicated profile (RENDER-05). Any user-set status accepts an optional description that surfaces in the fleet view (§3.8) as recall context. Skill plan/review signaling is gone with stage (see §3.6 for what remains of SKILL).

**STATE-01.** When the user invokes `status <value> [<description>]`, the plugin shall persist `<value>` as `override.status` and `<description>` (if any) as the session's description. `<value>` must be one of `idle`, `working`, `waiting`, `paused`, `wrapping`.
...
**CMD-04.** When the user invokes `status <value> [<description>]`, the plugin shall apply STATE-01..03 (the project/task snapshot in STATE-03 fires only for `paused`) and re-render. When the user invokes `pause [<note>]`, the plugin shall treat it as `status paused [<note>]` per STATE-07; when the user invokes `wrap [<note>]`, the plugin shall treat it as `status wrapping [<note>]` per STATE-08.
...
beacon writes to a small fixed set of surfaces of an iTerm2 window — three on every status change, plus the pane background in a mode state (`paused` / `wrapping`, RENDER-05). Every other surface is owned by Claude Code, the user's profile, or other tools, and beacon shall not touch them:
...
**BADGE-09a.** Two conditions take precedence over the BADGE-09 mapping and force a fixed color state regardless of the underlying provider chain. Precedence is a **mode status** (`paused` / `wrapping`, when set via override) > `pending-attention` — a mode is the most explicit user/session intent; pending attention demands action:

- A mode `override.status` (STATE-01) forces that mode's logical state (BADGE-10 / RENDER-05) — `paused` is a user-initiated halt, `wrapping` a deliberate post-work phase; both are distinct from being blocked on the user, and both own a dedicated profile.
...
**STATUS-BAR-01.** The `install` command shall write the beacon dynamic profile (carrying the status bar layout from STATUS-BAR-02 and the badge sizing from BADGE-13) into iTerm2's `DynamicProfiles` directory, inheriting from the user's currently-default profile, alongside its `beacon-paused` variant (RENDER-05 / §6.6) derived from the same layout. iTerm2 watches that directory and reloads dynamic profiles without restart, so this write succeeds even while iTerm2 is running. Filename and exact directory path are an iTerm2 contract documented in §6.
...
The mode pane backgrounds (RENDER-05) are delivered by the mode profiles rather than the badge/tab OSC: `paused` is a muted purple (`#3c3357`) that harmonizes with the comment-gray badge and carries a faint `||` watermark; `wrapping` is a muted green (`#2c4636`). Both are tunable in one place — the `MODE_PROFILES` table in the plugin script. The `wrapping` badge color is a distinct teal-green so a wrapping-up session does not read as a calm `ready` (green) one.
...
apply(state):
  ...
  logical_state = state.status if state.status in MODE_PROFILES  # paused/wrapping (BADGE-09a + RENDER-05)
...
4. Writes the `beacon` dynamic profile and its `beacon-paused` variant into iTerm2's `DynamicProfiles` directory (STATUS-BAR-01 / RENDER-05).
