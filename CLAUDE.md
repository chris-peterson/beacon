...
- **Claude Code status line (STATUSLINE-01)** — a footer row Claude Code renders from `beacon statusline` (opt-in, wired into `settings.json`). Terminal-agnostic and non-overlapping (Claude owns the row). Today it carries only the pause reason while paused (`⏸ <reason>`, colored), empty otherwise; the home issue #26 (URL OSC-8 link) and #18 (deliverables) extend.
...
| Render the Claude Code status-line row (pause reason) | `echo '{}' \| python3 scripts/beacon statusline` |
