...
- **Claude Code status line (STATUSLINE-01/02)** — a footer row Claude Code renders from `beacon statusline` (opt-in, wired into `settings.json`). Terminal-agnostic and non-overlapping (Claude owns the row), so it's where per-session *values* live. Segments joined by ` · `: the pause reason while paused (`⏸ <reason>`, colored), then the resolved URL as a clickable OSC-8 link. Nothing prints when every segment is empty. It reads `resolved.url` / `resolved.url_label` from state and **never** calls `resolve_url` — the row renders per prompt and that chain shells to git and possibly `gh`/`glab`.
...
| Render the Claude Code status-line row (pause reason + URL link) | `echo '{}' \| python3 scripts/beacon statusline` |
