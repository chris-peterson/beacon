...
**Coverage:** 195 spec requirement IDs, all implemented (0 Missing, 0 Contradicts).
...
A session resolves on **independent axes**, two of which were one `status`
...
RENDER-05). `permission` is hook-observed like activity and reduces to one
distinction, plan mode (PERM-01).

The old single field merged `mode` and `activity` by priority, so the declared
value won and the observed one was discarded: a session in `release` or `retro`
could not report that it was blocked on the user. Splitting them makes
`release · waiting` representable, and the surface assignment is what keeps both
readable at once — with many tabs open only one *pane* is visible, so the tab's
two slots take one axis each. Nothing arbitrates between them, which is why no
requirement states a precedence.

The permission axis reaches those same two slots without a precedence rule
either, because it **re-routes** rather than ranks (PERM-02, TITLE-07): while a
session is planning it takes the color, and the attention signal that color was
carrying leads the glyph slot. Both axes still reach the tab, where ranking one
over the other would have dropped one.
...
| PROV-07 | 1 | Covered | `url` chain; branch-slug match (`scripts/beacon` `_tack_url_for`); tier 0 the sibling-announced CR, recorded by `_read_announcements` off a `codes.bridgeai.anchor/cr.created` or `cr.updated` line, labelled by the announced `title` with the URL-derived `#<n>` beneath it, control characters stripped before storage (`_display_safe`, since the label reaches OSC-8), and scoped to the project root it was announced in (`_announced_cr_url`, `AnnouncedCrUrl`) |
...
| HOOK-01, 01a, 02, 03, 03a..03e, 08, 08a..08c, 09, 10, 12, 13, 14 (HOOK-11 retired) | 18 | All Covered | Hook handlers; HOOK-03/03b paint permission and idle identically — the sticky marker carries a constant, not the prompt type — while HOOK-03d reads the kind for the one decision that is not a paint: an `idle_prompt` on a stood-down session (STATE-15) records nothing, since a parked pane sits at an idle prompt by definition and red would become its resting state (`cmd_hook`, `IdlePromptOnAStoodDownSession`); HOOK-03's Notification hook is wired unmatched and reads the kind from the payload's `notification_type`, so the attention allowlist (`ATTENTION_NOTIFICATIONS` — MCP elicitations alongside the permission and idle prompts; the teammate kinds stay out because they report another session's block, not this pane's) lives in the handler rather than in the wiring, which could only carry the kinds it named (`NotificationKindsAreReadFromThePayload`); HOOK-08a's fresh-start wipe also stamps `session_started_at`, the window STATUSLINE-03 scopes acquisition to (`_wipe_session_for_fresh_start`); HOOK-09 disengages the pane on SessionEnd, handing the session name back to the interactive template *before* blanking the vars that template interpolates (`_disengage`); HOOK-10 emits the bundled ambient rules (`hooks/emit-rules.sh`, `keep-session-labeled`) at SessionStart; HOOK-11 retired with the `handoff` mode it was the only automatic writer of (STATE-14) — tack's session-close skill no longer moves the pane, and `_skill_invocation` / `SLASH_COMMAND_RE`, which existed for it alone, go with it; HOOK-12 hands a `claude` nested in a live pane its own bucket, keyed `claude-session:<id>` and marked `guest_of`, so a guest's SessionStart can no longer wipe the host's signals or repin its anchor (`_pane_held_by_other`, `_claim_guest_bucket`, `_guest_seed`, `ANestedSessionDoesNotTakeThePane`); HOOK-13 enters `release` off the release skill's invocation, resolved from both the `Skill` tool call and the typed slash command, and skipped when the mode is already held so a repeat cannot drop its note (`_invoked_skill`, `_enter_subscribed_mode`, `SKILL_MODE_SUBSCRIPTIONS`, `SubscribedSkillEntersItsMode`); HOOK-03e takes the same block from `PermissionRequest`, which arrives with the ask, where the Notification behind HOOK-03 is armed on a six-second unanswered-prompt timer that PreToolUse's `working` write outruns (`cmd_hook`, `PermissionRequestPaintsBlockedImmediately`); HOOK-08c republishes the branch slots alone from PostToolUse when the Bash command names a branch-moving git subcommand, so a push made early in a long turn stops painting `↑1` until Stop — gated on the subcommand because read-only git is most of what a session runs, and sharing one classifier with HOOK-08b so the two publishers cannot disagree (`_refresh_branch_chips_if_git`, `_branch_slot_pairs`, `MidTurnBranchRefresh`); HOOK-14 compares each installed entry point by the `scripts/beacon` **path** it reaches rather than a version string, which is what lets it see the `.zshrc` line and the profile buttons at all (`_pinned_surfaces`, `_freshness_drift`, `cmd_freshness`, `InstallFreshness`), and delivers the finding on `systemMessage` as well as `additionalContext` — the first is the only hook output Claude Code renders to the user, and the context it used to rely on reached only the model, which was free to skip it (`_freshness_banner`, `_freshness_summary`) |
| PERM-01..02 | 2 | All Covered | Observed permission mode; PERM-01 records the `permission_mode` every hook payload names and reduces it to one boolean, plan or not, leaving a recorded value in place when a payload omits the field — entering plan mode is a keystroke between turns and fires no hook of its own, so the next hook of any kind is the observation point (`cmd_hook`, `read_planning`, `_is_planning`, `PlanModeAwareness`); PERM-02 keeps it off every terminal surface, publishing it to the sessions view alone (`wip` state cell, the dashboard's `plan` tag) — colour is the only pre-attentive channel and answers urgency alone, the glyph slot holds the declared mode, and a row has the width for a word that a tab does not (`test_planning_leaves_the_tab_colour_to_the_activity`, `test_planning_leaves_the_tab_glyph_to_the_mode`) |
...
| CMD-01..09, 13..15, 17, 21, 23..31 (gap 10, 11; CMD-12, 16, 18, 19, 20, 22 retired) | 23 | All Covered | CMD-01 `show` reports each chained signal with its provider plus the two axes (no provider — one writer each); CMD-15 `json` now satisfies its own contract, emitting every signal with a provider, `mode` as a nested `{name, note}` tuple, and `activity` — the `task` + `task_provider` pair chris-peterson/anchor#2 was parked on; it drops `status`/`description` with no aliases, since a `status` alias would have had to re-merge the axes and hand every consumer the defect back; CMD-24 is `drop <ref>`, which takes one deliverable off the status-line row and remembers the removal in `deliverables.dropped` so route re-reads don't restore it (`cmd_drop`); CMD-08 install writes the base + mode dynamic profiles and takes the `--dir` that came off the retired `install-cli` (`cmd_install`); CMD-13 now specifies the wrapper `install` writes rather than a subcommand of its own — `install-cli` ran install's first two steps and was what the drift nudge named, which left the version-pinned `.zshrc` source line unrefreshed; CMD-25 is `/beacon:pause` (`commands/pause.md`, user-invocable only) and CMD-26 is `/beacon:install-beacon` (`commands/install-beacon.md`), the only door to the newly-installed plugin root, named by `hooks/cli-freshness.sh`; CMD-16 retired in 2.0 with the branch-review feature (chip + `beacon review` subcommand + the moor/anchor soft deps); CMD-18 (the `/beacon:session-mode` shim CMD-19/20/22 folded into in 2.0, issue #23) is itself retired — its one live clause was staying model-invocable for a skill caller that never materialized, since a skill reaches `beacon release` in one shell call; CMD-21 is `data-dir` (renumbered 2026-07-06 from a duplicate CMD-16), now a contract for env-less callers rather than the shell's path to it; CMD-27 is `shell-init` (`cmd_shell_init`), the one zsh-sourceable block carrying every source-time value `shell/beacon.zsh` needs — the shell caches it and regenerates only when the script, the data-dir pointer, or the user config is newer, so a new terminal spawns no interpreter in the steady state; CMD-23 is `refresh-iterm-profiles`, the profile-only re-render that applies a changed button label, a moved interpreter, or a GUI-edited profile (`cmd_refresh_iterm_profiles`) — renamed off the `install-*` prefix, which now means the bootstrap only; it is never a first-install step, since `install` calls the same `install_dynamic_profile` renderer; both call sites now check that iTerm2 will *load* what they wrote (`_render_shadowed_warning`) rather than reporting the write alone — a render that writes five files and reports five files, while iTerm2 skips every one, reads as success from the command's own output — and `--remigrate` is what clears it, invoking CLI-19's re-arm of iTerm2's own migration (`REMIGRATE_COMMAND`, named by the refusal, the install step, and the doctor row alike), opt-in because it costs an iTerm2 restart; CMD-28 is `layout` (`cmd_layout`), the plugin front door for CLI-18's app-wide layout audit: `beacon` is the only interface a user is expected to type, and the CLI is told which name to advertise (`BEACON_LAYOUT_COMMAND`) so its own advice, `install`'s drift line, and the docs all name `beacon layout` — it stays separate from CMD-23 because the two write disjoint things and only the app-wide half needs an iTerm2 restart, leaving CMD-23's no-advisory clause intact; CMD-29 is `doctor` (`cmd_doctor`), the front door to DIAG — install and environment checks plus the recorded error log, exiting non-zero so it can gate a health check; CMD-30 marks `--version` `-dev.g<short-sha>` (plus `.dirty`) from a working tree (`_version_display`, `_is_dev_install`) — a git dir is present in exactly one of the two copies, and without the marker a released and an unreleased beacon of the same number read alike, and `-v` is its alias; CMD-31 is the `task` / `project` shorthand for the corresponding `set` — the two fields a session relabels while it works, typed from an ambient rule on nearly every session, which is why they carry a shortcut the other override fields do not (`cmd_task`, `cmd_project`) |
| WIP-01..20 (incl. 18a, 18b) | 22 | All Covered | Cross-session introspection / export; WIP-06 `prune` sweeps the per-pane cache (`cwd-*`, `engaged-*`, and the `url-*` files 2.0 retired) on the same `--since` cutoff as state, by each file's own mtime since the pane GUID isn't recoverable from a session hash (`_prune_cache`), and the same sweep now also runs unprompted at SessionStart behind a once-a-day stamp (`_sweep_stale_state`, `_prune_state`, `cache/last-prune`, `AutomaticStateSweep`) — as a verb alone it went unrun, so turn text accumulated for every pane ever opened; running it from a hook is what put it under PERF-01's scan discipline, so `_prune_state` resolves activity through `_session_mtimes`' single pass and both sweeps read their directory a fixed number of times whatever the session count (`test_sweep_cost_does_not_grow_with_session_count`); WIP-01 emits `focusable` (FOCUS-03) + `icon` (PROV-08) + `latest_turn` (WIP-11); WIP-08 is the `/icon/<hash>` serve route; WIP-09 emits the session→tack bound `tacks` (route-qualified, existing/emerging), filled in from the route document since summary / status / refs are all mutable after the binding fires; WIP-02 takes the route from tack's own `session.started` / `session.ended` announcements (`announced.route`, stamped with the Claude session id it was announced for) and asks `tack tree <slug> --json` what that route currently holds — the route-file scan and the pin / branch / project-name guesses it replaced are gone, along with `TACK_HOME` (`_bound_route_record`, `_route_doc_for`, `_bound_tack_records`, `AnnouncedTackRoute`); WIP-10 is the bundled reference dashboard `serve` hosts at `/` (`dashboard/index.html`); WIP-11 is the auto-derived `latest_turn` (human prompt / agent reply), written at hook time, ellipsized to card width by the dashboard; WIP-01 carries `mode` as the same nested `{name, note}` tuple CMD-15 emits, so beacon's two published payloads describe the value identically (`_record_mode` is the one accessor the sessions-view renderers read it through); WIP-12: every sessions-view consumer shows *every* axis it has a value for — the dot carries `color_state`, the mode carries its own glyph, and the text views render `plan·release·waiting` (`_wip_state_cell`), since a session blocked on the user is blocked whatever mode it declared and whatever it is permitted to do; a consumer classifying a session as blocked reads `activity` / `pending_attention`, not the `color_state` a planning session spends on the other axis (`blockedOnUser` in `dashboard/index.html`); WIP-13 emits `agent_color` (sessions-view identity pill only, never painted); WIP-14 persists `latest_turn_full` + serves it at `GET /turn/<hash>` for card expansion; WIP-15 collapses same-project sessions into a z-stack (newest front, raise on demand); hovering or tab-focusing a tucked card brings the whole card forward in place, replacing the floating task+turn tooltip — a partial re-rendering of a card the reader can simply be shown (it comes forward in place because the ~11-13px peek means translating it would move it out from under the pointer and oscillate); WIP-16 auto-groups the sessions by route group (groupless in an unlabeled bottom section, no toggle); WIP-17 gives moded cards the pane-analog treatment (muted tint + centered watermark) on `mode` while the dot and accent bar answer to `color_state`, so a moded session that goes `waiting` keeps its treatment *and* turns red; it stays out of the attention band because the band's glow would paint over the only place the mode is legible, not because the mode outranks the signal; WIP-18 is the service-wide access model every route now carries — loopback `Host` plus a loopback-or-allowlisted `Origin`, and `Access-Control-Allow-Origin` echoed only for an origin that passed (`do_GET`, `_cors_origin`, `_send_cors`, `_allowed_origins`, `ReadRouteAccessTest`) — which the reads previously had no part of, answering `*` to any page with the transcript-derived turn text of every session on the machine; WIP-18a records each refusal to the DIAG-01 log as `serve.origin` or `serve.host`, once per distinct value per process, so a dashboard that reads as a dead feed has an entry naming the origin to allowlist rather than only a silent refusal (`note_refusal`, `_ADVICE`); WIP-18b answers a CORS preflight under the same access model as the request it precedes, since a preflight refused is the request refused (`do_OPTIONS`); WIP-19 is the document picture-in-picture panel — always-on-top, seeded with the sessions blocked on the user, a row click focusing rather than expanding because a panel sized to float has nowhere to expand into (`dashboard/index.html`: `openPip`, `syncPip`); WIP-20 renders the expanded turn as the Markdown it was written as, escaping the text before formatting and admitting a link only for `http`/`https`/root-relative/fragment — the turn is transcript-derived and reaches the origin holding every session's text, so the escape is the control, and it is the deliberate opposite of WIP-11's marker-stripped excerpt (`renderMarkdown`, `renderInline`) |
...
| DIAG-01..08 | 8 | All Covered | Error log + `doctor` (`log_error`, `read_error_log`, `_doctor_checks`, `cmd_doctor`). DIAG-01 records swallowed external-command failures as JSONL at `<DATA_DIR>/logs/errors.log`, keyed on non-zero exit as well as raised exception — the render CLI reports a failed iTerm2 operation the first way, which is how a broken `set-name` went a whole session unnoticed; DIAG-02/03 keep each record single-line and under `PIPE_BUF` for atomic concurrent appends and trim the file to its tail; DIAG-04 scopes recording to shelled-out failures so ordinary absences don't bury real ones; DIAG-05 probes the data-dir pointer, state dir, wrapper, the entry points still reaching the loaded plugin root (HOOK-14's check on demand — the row names which path is pinned, where the SessionStart banner has room only to say that one is), status line, and — where the adapter applies — iTerm2 install/run state, the dynamic profiles, whether the `<DATA_DIR>` those profiles' action buttons were rendered against is still the one hooks write (`_doctor_profile_paths` / `_profile_baked_paths`, which read the baked paths back out of each profile), whether iTerm2 is still *loading* those profiles rather than holding saved copies that shadow them — 3.7.0 reads an `Is Dynamic Profile` key where 3.6.x read a `Dynamic` tag, so a profile saved before the upgrade takes the Guid its own file declares and the loader skips the file (`_doctor_profiles_shadowed` / `_iterm_prefs`, gated on the one-shot migration iTerm2 records, since on an older iTerm2 the key's absence carries no finding), and whether this session's recorded pane handle still resolves through Apple Events (reporting a handle that is not a session id by shape rather than quoting it, since the report lands in a terminal); DIAG-08 keeps an adapterless box healthy rather than failing |
| COLOR-01 | 1 | Covered | `--color` + `NO_COLOR` / `FORCE_COLOR` precedence |
| FOCUS-01..04 | 4 | All Covered | Dashboard focus: `POST /focus` route + `_focus_session`, `iterm_session_id` handle (FOCUS-02), `focusable` in payload (FOCUS-03). FOCUS-02 now also constrains the handle's *shape*, established once at the read boundary (`_read_iterm_handle`) so a value that fails it is no handle to every consumer at once — `focusable` cannot advertise one `/focus` would refuse (`ITERM_GUID_RE`, `_record_focus_handle`, `ItermHandleGuard`); `_iterm_session_reachable` re-checks at the AppleScript boundary and `doctor` reads the raw value deliberately, being the one consumer whose job is to report that it is malformed; FOCUS-04 is now WIP-18's model plus the body drain a mutating route's rejection needs |
| FORGET-01..03 | 3 | All Covered | Dashboard forget: `forget <hash>` CLI + `POST /forget` route → `_forget_session` (FORGET-01), hex-hash guard (FORGET-02), the same WIP-18 + body-drain model `/focus` carries (FORGET-03); tests in `ForgetTest` |
...
| CLI-01..03, 06..12, 14..20 (gap 13; CLI-04/05 retired) | 17 | All Covered | CLI-15 is `set-name`, the Apple Events title path (`cmd_set_name` in `bin/beacon-iterm`) — the id was reused for it when the spec moved to the root, the third of the three live reuses below; CLI-16 is the `--help` usage table; CLI-17 `focus` via osascript (`cmd_focus` in `bin/beacon-iterm`); CLI-18 is `configure`, one of the two paths that write an iTerm2 preference — explicit, user-invoked, quit-write-relaunch (`cmd_configure`), never reachable from a hook or render; its nine audited settings are a spec table (`RECOMMENDED_LAYOUT` in `bin/beacon-iterm`), the newest being `CompactMinimalTabBarHeight = 90` — the per-tab height iTerm2 3.7.0 gives a Minimal side strip in place of `DefaultTabBarHeight`, whose 38pt default clips line 2 of the label — and `TabStyleWithAutomaticOption = 5` — `TAB_STYLE_MINIMAL` in iTerm2's own enum, read from the enum because the Theme popup's item order is not it, adding `HideTab=0` (iTerm2 hides the tab bar at one tab per window, taking the whole per-pane signal with it) and moving `StatusBarPosition` to `0` (the bottom of the pane is where Claude Code renders STATUSLINE-01); CLI-18's running check asks iTerm2 through Apple Events rather than `pgrep -x iTerm2`, which never matches it (macOS matches `-x` against the full executable path) — the false negative inverted the one branch the orchestration turns on, writing into a live iTerm2 that then restored the old values on quit, and the detached helper polled the same way; the audit now also qualifies an aligned reading while iTerm2 is running, since `defaults read` answers from a plist the running app will overwrite from memory; the detached helper's log is created by `mkstemp` (exclusive, 0600, path printed) rather than at a guessable name in the temp dir, which another local user could pre-create as a symlink for the truncating open to follow (`_configure_write`, `test_helper_log_path_is_unpredictable_and_owner_only`); CLI-19 is `remigrate-profiles` (`cmd_remigrate_profiles` in `bin/beacon-iterm`), the other writer, and it writes one boolean: iTerm2 3.7.0 reads an `Is Dynamic Profile` key where 3.6.x read a `Dynamic` tag and ships a one-shot migration for the gap (`ITAddressBookMgr` init, iTerm2 issue 12806), so a run that left a profile unflagged has closed a gate that never reopens — clearing `NoSyncMigratedDynamicProfileTagToFlag` re-arms it and iTerm2 repairs its own profiles on the next launch. beacon deliberately does not write `New Bookmarks`: doing iTerm2's job there is destructive where one boolean asks. Shares CLI-18's orchestration (`_run_after_quit`, factored out of `_configure_write`); CLI-20 is `reset-layout`, which reports every app-wide pref beacon renders through that carries a value — grouped by the surface it reaches — and clears them to iTerm2's own defaults given `--write`, so the recommendation can be audited from a machine that is not already holding its gaps shut (`RESET_FAMILIES`, `cmd_reset_layout`, `ResetLayoutToStock`) |
| BADGE-01..07, 09, 09a, 11..15 (BADGE-08, 09b, 10 retired) | 14 | All Covered | Badge text + color + engagement; BADGE-09 maps `activity` to the three-value stoplight and no mode reaches it (`ACTIVITY_TO_COLOR_STATE`, `COLOR_PALETTE`); BADGE-09a keeps only the `pending-attention` clause — the mode clause above it is exactly what suppressed the interrupt signal; BADGE-10 retired with the per-mode color (a paused session now reads by its `⏸` glyph and pane background instead); BADGE-11 leaves the badge *text* undecorated — the mode's marks live on their own surfaces; BADGE-15 is **not** the retired watermark — the id was reused for the opt-in badge gate (off by default, `"badge": "on"` in the user config, read via `config-get` at all three paint sites); BADGE-09b retired with the colour it claimed — the permission axis paints no terminal surface (PERM-02), since colour is the only pre-attentive channel and spending it on anything but urgency costs the one signal that must read across every tab at once |
| TITLE-01..06 (incl. 05a; TITLE-07 retired) | 7 | All Covered | OS window title via the iTerm2 session *name* (Apple Events `set-name`; profile `Allow Title Setting: false`); TITLE-01 interactive panes fall back to the cwd (`beacon_title` = project else cwd); TITLE-02 records that iTerm2 implements the session name as a session-scoped override of the session's copy of the profile `Name` key (§6.10 caveat 7), so an engaged pane reads its profile name back as the raw template while `set-profile` still matches; TITLE-04 one-shot re-assert on the first turn boundary reclaims the title from the shell's backgrounded launch write, and disengagement returns the name to `beacon_title` ahead of blanking the badge user vars, since the shell's own write never re-runs (`test_name_handback_precedes_blanking_the_title_vars`); TITLE-05 is the two-line tab label (`TITLE_FORMAT`, `<b>project</b>` over the indented `beacon_task_nl`), whose line 1 doubles as the single-line OS window title; TITLE-06 leads line 1 with the declared mode's glyph via `beacon_title_prefix` (`MODE_SPECS` — `⏸` `🚀` `📋` `🏁`), which is the mode's *only* cross-tab surface now the tab color reports activity; until 2.5.0 only `paused` marked the title, leaving four of five modes legible from another tab by color alone; TITLE-05a gives a stood-down mode's note line 2 in place of the task, the note's only cross-tab surface — the status line it otherwise relies on exists solely in the focused pane, and a halted session has no live task to displace (`resolve`, `StoodDownModeNoteOnLineTwo`); TITLE-06 now also covers the dev lead — a space rather than a mark, so every label starts in the same column while the default stays the absence of a declaration and the rare marks keep their salience (`DEV_TITLE_LEAD`, `TitleLeadIsConstant`); TITLE-07 retired one release after it landed, having existed only to carry the attention signal the retired BADGE-09b displaced from the colour channel |
...
| TAB-01..04 | 4 | All Covered | TAB-01: OSC `tab-color` carries the resolved color state and nothing else (`_color_state_for`) — no mode reaches it, which is what lets a moded session still report that it needs the user; the state is `activity`'s except while plan mode holds the channel (BADGE-09b). TAB-04 paints that state's hue under a per-state area weight (`TAB_MUTE`, `_tab_hex`, `_mute_hex`, `TabAreaWeighting`), because Minimal (pinned by CLI-18) fills the whole tab with it; the badge keeps the unweighted hue, being small and over terminal output. Weighted per state rather than globally: uniform dimming calmed the strip and flattened it, and the measured ladder ran backwards beforehand (`busy` 8.37 contrast vs `blocked` 4.54) |
| THEME-01, 02, 02a, 03 | 4 | All Covered | Dracula palette; THEME-02 is the three activity hexes (`COLOR_PALETTE`), each with the tab weight it paints at (TAB-04) and THEME-02a the per-mode background + watermark + blend (`MODE_SPECS`) — the axes never share a table because they never share a surface; `retro`'s blend rises to 0.25, since flat geometry carries less ink than the illustration it replaced |
...
**⚠ Resolved 2026-09-15** (seven divergences found by that day's spec-sync;
the code was right in every case, so each was settled by correcting the spec):

- ~~**Branch-chip colors**~~ — the profile renders `default` slate `#6272a4`,
  `clean` cyan `#8be9fd`, `diverged` yellow `#f1fa8c`, `untracked` orange
  `#ffb86c`. STATUS-BAR-03 said so; STATUS-BAR-02, THEME-01's prose and
  THEME-03's table still described the pre-hybrid green/orange/gray scheme, and
  THEME-03 called its own hexes authoritative. All three corrected, and the
  default-branch slot added to the table it had never appeared in. A test now
  reads both files and compares them (`test_the_spec_chip_table_matches_the_profile_it_documents`)
  — the table's claim of authority is what had been resting on nothing.
- ~~**BADGE-13's transparency half**~~ — dropped. The badge color arrives by the
  same OSC that carries every state color (CLI-10) and that control has no alpha;
  delivering transparency would mean writing the color through the profile as
  well, splitting one value across two writers.
- ~~**STATE-10's iTerm2 gate**~~ — removed from the requirement. The escape
  sequence is the standard one every terminal answers, so conditioning the clear
  on which terminal is attached would have required sniffing this path has no
  other reason to do.
- ~~**DIAG-04's scope**~~ — widened to cover failures of beacon's own writes that
  it swallows under NFR-06, not only shell-outs. The case that prompted it: a
  sweep that cannot write its stamp file stops pruning silently, which is the
  failure the log exists to surface.
- ~~**WATCH-01's `q`**~~ — the requirement promised `q` unconditionally while the
  fallback path offers only an interrupt. Restated as the affordance rather than
  the mechanism: an interrupt quits on every platform, and `q` also quits where
  the terminal can hand over a single keypress. The earlier draft of this fix
  named the POSIX terminal API, which would have written the mechanism into the
  contract — the same defect the quality audit flagged in fifteen other
  requirements.
- ~~**TITLE-04's polling**~~ — the requirement had the shell's backgrounded job
  poll for the engagement marker; it checks once. The requirement now says so and
  carries the reason the shell gives for not waiting: a deferred write lands last
  and strands an engaged pane on the interactive title, and every ordinary tab
  would pay the wait for a marker it never receives.
- ~~**PROV-08's icon ordering**~~ — `ICON_CANDIDATES` orders by location before
  format, so `docs/favicon.png` outranks a root `favicon.svg`. Requirement
  reworded to "SVG preferred over raster **within each location**". Decided in
  favor of the code: at dashboard-favicon size the format barely shows, and where
  a project puts its icon is the better statement of which icon it means.
...
### 2026-09-15 — spec-sync --to-spec

+7 IDs, 188 → 195, and 7 divergences recorded above rather than resolved.

Ledger first: HOOK-14 now delivers the freshness finding on `systemMessage` as
well as `additionalContext`, and DIAG-05 gained the `entry points` row that runs
the same check on demand. CLI-20 (`reset-layout`) and WIP-18a (refusals recorded
to the error log) shipped in 2.13.0 and were never ledgered; both were read
against the code and recorded Covered.

Then a full-domain sync captured four behaviors the code had and the spec did
not — WIP-18b (CORS preflight), WIP-19 (the pop-out panel), WIP-20 (the expanded
turn rendered as Markdown, escaped, with link schemes restricted) and CMD-31
(the `task` / `project` shorthand) — and amended CLI-02, PROV-08, DIAG-07 and
CMD-30 to describe what their code already does. Six requirements were restated
in EARS form without changing what they require: THEME-02, THEME-02a, STATE-13,
DUMP-04, WIP-02, WIP-10.

Seven divergences between the spec and the code were then settled, all seven by
correcting the spec — the branch-chip colors across three sites, BADGE-13's
transparency clause, STATE-10's iTerm2 gate, DIAG-04's scope, WATCH-01's quit
key, TITLE-04's polling, and PROV-08's icon ordering. They are recorded under
Open items.

Every requirement also became its own `#### ID` heading, so `SPEC.md#res-01`
addresses one requirement instead of a 1400-line file. The numbered category
headings stayed as they were: AGENTS.md cross-references §4.1, §4.8 and §6.10,
and renaming the categories would break those for an anchor nobody links to.

### 2026-09-10 — Coverage refresh (spec-status)

+1 ID, 187 → 188. CLI-19 (`remigrate-profiles`) re-arms the one-shot migration
iTerm2 3.7.0 repairs a stopped-loading profile with. DIAG-05 detects that state
and CMD-23's `--remigrate` clears it, so the detection and the remedy ship
together. CLI-18 amended in place: it is one of two preference writers rather
than the only one.

### 2026-09-03 — Coverage refresh (spec-status), third pass

−2 IDs, 189 → 187. BADGE-09b and TITLE-07 retired one release after landing:
colour is the only pre-attentive channel, so the permission axis takes no
terminal surface (PERM-02 rewritten) and the `?` that bought back what the
colour displaced has nothing left to do. TITLE-06 absorbed the dev lead — a
space, not a mark. §4.8 and TITLE-05 corrected: the Minimal style CLI-18 pins
does not draw the OS window title in the frame and it reaches neither Mission
Control nor ⌘-Tab, so line 1 is shaped for the tab strip.

### 2026-09-03 — Coverage refresh (spec-status), second pass

+1 ID, 188 → 189: TAB-04 weights each color state's hue per state for the tab
alone, since CLI-18 now pins iTerm2's Minimal style and Minimal fills the whole
tab with the color. TAB-01 and THEME-02 amended in place, and CLI-18's audited
set grew to eight with `TabStyleWithAutomaticOption`.

### 2026-09-03 — Coverage refresh (spec-status)

+4 IDs, 184 → 188, all Covered: PERM-01..02 open a new §3.3a for the hook-observed
permission axis, BADGE-09b gives a planning session the tab color ahead of the
activity stoplight and its sticky marker, and TITLE-07 takes the displaced
attention signal onto the tab glyph as `?`. The context paragraph's axis count
went with them (two → three), and the TAB-01, THEME-02 and WIP-12 notes were
amended in place where they named the stoplight as the whole of the color channel.

### 2026-09-02 — Coverage refresh (spec-status)

No ID change; 184 IDs, all Covered. WIP-02, WIP-09 and PROV-07's tack tier
amended in place: the route a session is working now comes from tack's
`session.started` / `session.ended` announcements rather than a scan of
`$TACK_HOME/routes/*.yaml` for the session id, and the four location heuristics
below that scan (`.tack` pin, branch, project name whole and by basename) are
gone. What the route currently holds is read from `tack tree <slug> --json`,
which is grammar tack publishes, where the route files were a schema it can
change silently. `announced.route` joins the state listing.

### 2026-09-02 — Coverage refresh (spec-status)

+1 ID (HOOK-08c, Covered): a PostToolUse whose Bash command names a
branch-moving git subcommand republishes the branch slots on the spot, where
HOOK-08b's per-turn publish had left a mid-turn push painting `↑1` for the rest
of the turn. 183 → 184 IDs, all Covered. STATUSLINE-02, STATUSLINE-03, PROV-07,
and HOOK-08a amended in place for the same event work: three of anchor's new
announcements (`cr.merged`, `issue.created`, `release.created`) now feed the row
directly, an announced CR publishes the link on arrival rather than at Stop, and
the fresh-start wipe covers the `announced.*` fields it was scoped to all along.

### 2026-09-01 — Coverage refresh (spec-status)

+1 ID (HOOK-03e, Covered): `PermissionRequest` joins the `Notification` hook as
a writer of `activity = waiting` + `pending-attention`, closing the six-second
window in which a blocked session still painted its tab busy. 182 → 183 IDs,
all Covered. HOOK-03 and HOOK-03d amended in place: the Notification hook is
unmatched and reads `notification_type` from the payload, so the attention
kinds are an allowlist in the handler rather than a matcher per kind in the
wiring.

### 2026-08-31 — Coverage refresh (spec-status)

+1 ID (WIP-18, Covered): the loopback + origin access model now governs every
`serve` route, not only the mutating two. 181 → 182 IDs, all Covered. FOCUS-02
(handle shape re-checked at every read), FOCUS-04 / FORGET-03 (restated on top
of WIP-18), WIP-06 (the age sweep runs unprompted at SessionStart), CLI-18 (the
helper log is created exclusively) and §6.2 (0700 dirs / 0600 files) amended in
place; all still Covered, no other count change.

### 2026-08-30 — Coverage refresh (spec-status)

+1 ID (HOOK-13, Covered): the release skill's invocation now enters `release` mode, read from both
shapes an invocation arrives in. 180 → 181 IDs, all Covered.

### 2026-08-30 — Coverage refresh (spec-status)

+1 ID (HOOK-12, Covered): a `claude` started inside a live pane now takes its
own state bucket instead of the host's. 179 → 180 IDs, all Covered. WIP-01
amended in place — the row reads a `project` override above the anchor, as the
render chain does; still Covered, no count change.
