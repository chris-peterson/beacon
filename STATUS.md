...
| TITLE-01..06 (incl. 05a) | 7 | All Covered | OS window title via the iTerm2 session *name* (Apple Events `set-name`; profile `Allow Title Setting: false`); TITLE-01 interactive panes fall back to the cwd (`beacon_title` = project else cwd), or to `🔗 <host>` while ssh holds the prompt (SSH-01); the interactive template gained a second slot for the remote cwd, and `test_interactive_title_format_matches_the_shell` now holds its two copies together — the comment beside each had always said they must stay in step, but nothing enforced it; TITLE-02 records that iTerm2 implements the session name as a session-scoped override of the session's copy of the profile `Name` key (§6.10 caveat 7), so an engaged pane reads its profile name back as the raw template while `set-profile` still matches; TITLE-04 one-shot re-assert on the first turn boundary reclaims the title from the shell's backgrounded launch write, and disengagement returns the name to `beacon_title` ahead of blanking the badge user vars, since the shell's own write never re-runs (`test_name_handback_precedes_blanking_the_title_vars`); TITLE-05 is the two-line tab label (`TITLE_FORMAT`, `<b>project</b>` over the indented `beacon_task_nl`), whose line 1 doubles as the single-line OS window title; TITLE-06 leads line 1 with the declared mode's glyph via `beacon_title_prefix` (`MODE_SPECS` — `⏸` `🚀` `📋` `🏁`), which is the mode's *only* cross-tab surface now the tab color reports activity; until 2.5.0 only `paused` marked the title, leaving four of five modes legible from another tab by color alone; TITLE-05a gives a stood-down mode's note line 2 in place of the task, the note's only cross-tab surface — the status line it otherwise relies on exists solely in the focused pane, and a halted session has no live task to displace (`resolve`, `StoodDownModeNoteOnLineTwo`) |
| SSH-01..10 | 10 | All Covered | An ssh'd pane's identity is the host, not the local directory it was launched from. SSH-01 composes `🔗 <host>` into `beacon_title` from a `preexec` hook — the last moment the local shell runs before ssh stops the prompt — parsing the display host out of argv fork-free (`_beacon_ssh_target`, SSH-03), skipping a trailing remote command and `-f` but painting `-N` (SSH-04); SSH-02's restore invalidates every sentinel rather than diffing, because a remote publisher may have written any slot and the local shell cannot see the screen (`_beacon_invalidate_sentinels`, extracted from `_beacon_chpwd` so the list has one home) — a diff there short-circuits and strands the tab on the host, which `test_the_ssh_sentinel_is_invalidated_on_return` pins; SSH-05..07 are `shell/beacon-remote.sh`, one POSIX file for zsh and bash whose non-interactive guard is the load-bearing line (escapes on a non-interactive channel corrupt `scp`/`rsync`/`sftp`/`git`-over-ssh), publishing only the slots the local half leaves it and never the two axes' OSC; SSH-08 is `ssh-install` / `ssh-uninstall` / `--print`, whose rc splice is a pure text function tested as one (`_splice_ssh_block`, `SshInstallEditsTheRemoteRc`) and which is unreachable from any hook or render; SSH-09/10 stop both buttons acting on the stale local directory (`ButtonsDuringAnSshSession`), `↗ code` fetching the remote path through CLI-19 because an action component cannot interpolate a user var though Apple Events can read one (§6.10 caveat 3) |
