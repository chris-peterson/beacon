...
| PROV-07 | 1 | Covered | `url` chain; branch-slug match (`scripts/beacon` `_tack_url_for`); tier 0 the sibling-announced CR, recorded by `_read_announcements` off a `codes.bridgeai.anchor/cr.opened` line and scoped to the project root it was announced in (`_announced_cr_url`, `AnnouncedCrUrl`) |
